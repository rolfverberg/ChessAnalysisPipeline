...
#                detector.mask_ranges = None
    ...
    def _calibrate(self):
        ...
        for energies, mask, mean_data, (low, upp), detector in zip(
            ...
            if self.interactive or self.save_figures:
                ...
                axs[1,0].plot(
                    energies[mask], results['residual'], c='C1', label=label)
                axs[1,0].legend()
                ...
                axs[0,1].plot(
                    ...
                    label='Peak strains')
                axs[0,1].axhline(
                    strain, ls='--', c='C1', label='Unweighted mean')
                axs[0,1].legend()
    ...
    def _direct_bragg_peak_fit(
        ...
        self._peak_fit_results[detector.get_id()] = {
            ...
            'independent_dimension': {'name': 'Energy', 'unit': 'keV'},
        }
...
class StrainAnalysisProcessor(_BaseStrainProcessor):
    ...
    def _add_fit_nxcollection(
        ...
        for hkl, material_name in zip(hkls, material_names):
            ...
            nxcollection = nxdetector[material_name]['unconstrained']
    ...
    def _create_animation(
        ...
        if self.save_figures:
            self._animation.append(
                ((ani, 'gif'), f'{detector_id}_strainanalysis_fits'))
        plt.close()
    ...
    def _get_nxprocess(self, nxentry, calibration_config):
        ...
        for energies, mask, nxdata, detector, peak_fit_info in zip(
            ...
            self.logger.debug(
                f'Setting up NXdata group for {detector.get_id()}')
            det_nxdata = nxdetector.data
            if 'axes' in det_nxdata.attrs:
                if isinstance(det_nxdata.attrs['axes'], str):
                    det_nxdata.attrs['axes'] = [
            ...
            # Setup the NXcollection objects for the per material peak
            # fitting results
            for name in set(material_names_fit):
                nxdetector[name] = NXcollection()
                nxcollection = nxdetector[name]
                nxcollection['uniform'] = NXcollection()
                nxcollection['uniform'].strain = NXfield(
                    dtype=np.float64,
                    shape=(num_points,),
                    attrs={'long_name': 'Strain from peak fit (\u03B5)'},
                    maxshape=(None,), chunks=(1,))
                nxcollection['unconstrained'] = NXcollection()
                nxcollection['unconstrained'].strain = NXfield(
                    dtype=np.float64,
                    shape=(num_points,),
                    attrs={'long_name': 'Strain from peak fit (\u03B5)'},
                    maxshape=(None,), chunks=(1,))
                nxcollection['unconstrained'].strain_stdev = NXfield(
                    dtype=np.float64,
                    shape=(num_points,),
                    attrs={'long_name':
...
#                'values': component_strain,
        ...
        for energies, mask, mean_data, nxdata, detector in zip(
            ...
            if self.json_results:
                raise RuntimeError('self.json_results needs testing')
                ...
                for j, (hkl, material_name) in enumerate(zip(
                    ...
                    for fitparam in fitparams:
                        ...
                        for fitquant in fitquants:
                            key = str(
                                f'{det_id}/{material_name}/unconstrained/'
        ...
        if self.config.calc_rosette_strains:
            raise RuntimeError('Rosette strains needs testing')
